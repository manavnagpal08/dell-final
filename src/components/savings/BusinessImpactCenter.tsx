...
const VIEWS = ['Cost Savings', 'ROI Growth', 'Downtime Reduction', 'Asset Protection', 'Energy Savings'];
...
export function BusinessImpactCenter() {
    ...
    if (loading || !data) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-400">
          ...
          <p className="text-[10px]">AI Analytics...</p>
        </div>
      );
    }
    switch(activeView) {
      case 'Downtime Reduction':
        return (
          <BarChart data={data['Maintenance Efficiency']} margin={{ top: 5, right: 5, left: -25, bottom: -5 }}>
            <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#64748b', fontWeight: 600 }} dy={5} />
            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#94a3b8' }} />
            <Tooltip content={<CustomTooltip />} cursor={{ fill: 'rgba(241,245,249,0.5)' }} />
            <Bar dataKey="val" radius={[0, 0, 3, 3]} isAnimationActive={true} animationDuration={1000} barSize={40} fill="#3b82f6" />
          </BarChart>
        );
      case 'ROI Growth':
      case 'Energy Savings':
        return (
          <LineChart data={data[activeView === 'Energy Savings' ? 'Device Health' : 'Cost Savings']} margin={{ top: 5, right: 5, left: -25, bottom: -5 }}>
            <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" />
            <XAxis dataKey={activeView === 'Energy Savings' ? 'day' : 'month'} axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#64748b', fontWeight: 600 }} dy={5} />
            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#94a3b8' }} />
        ...
        return (
          <div className="flex items-center justify-center h-full relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
              <span className="text-xl font-black text-emerald-500">{data['Power Utilization']?.[0]?.val || 88}%</span>
      ...
      default:
        ...
        return (
          <AreaChart data={data['Cost Savings']} margin={{ top: 5, right: 5, left: -25, bottom: -5 }}>
            ...
            <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 9, fill: '#94a3b8' }} />
            <Tooltip content={<CustomTooltip />} />
            <Area type="monotone" dataKey="val" stroke="#8b5cf6" strokeWidth={2} fillOpacity={1} fill="url(#colorCostArea)" isAnimationActive={true} animationDuration={1000} />
          </AreaChart>
        );
    }
  };
  ...
  return (
    <div className="bg-white/80 backdrop-blur-sm border border-slate-200/60 rounded-xl p-4 h-[220px] shadow-[0_2px_8px_-3px_rgba(6,81,237,0.05)] flex flex-col relative z-20">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center gap-2">
          <div className="p-1 bg-indigo-50 rounded text-indigo-600">
            <Briefcase className="w-3.5 h-3.5" />
          </div>
          <h3 className="text-sm font-bold text-slate-800 tracking-tight">Business Impact Center</h3>
        </div>
